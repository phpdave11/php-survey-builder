<div class="question" data-question_id="<?php if (empty($question)) echo 'QUESTION_ID'; else echo $question->getUniqueId(); ?>" data-question_number="<?php if (isset($i)) echo $i + 1; ?>">
  ...
  <h4>Question <span class="question_number"><?php if (isset($i)) echo $i + 1; ?></span></h4>
  <div>
    ...
    <div class="required_container">
      <input type="checkbox" id="is_required_<?php if (empty($question)) echo 'QUESTION_ID'; else echo $question->getUniqueId(); ?>" name="is_required[<?php if (empty($question)) echo 'QUESTION_ID'; else echo $question->getUniqueId(); ?>]" value="1"<?php if (!empty($question) && $question->is_required == 1): ?> checked="checked"<?php endif; ?> />
      <label for="is_required_<?php if (empty($question)) echo 'QUESTION_ID'; else echo $question->getUniqueId(); ?>">Required question</label>
    </div>
  </div>
  <div>
    <label>Question text:</label>
    <input type="text" class="question_text" name="question_text[<?php if (empty($question)) echo 'QUESTION_ID'; else echo $question->getUniqueId(); ?>]" value="<?php if (!empty($question)) echo htmlspecialchars($question->question_text); ?>" />
  </div>
  <div class="choices_container"<?php if (empty($question) || !in_array($question->question_type, array('radio', 'checkbox'))): ?> style="display: none"<?php endif; ?>>
    <h4>Choices</h4>
